...
3. Warnings 
	- Program compiles successfully, but compiler give warning regarding possible chances of program failure at runtime.
...
###	Input and Output
- In the following statement, the data i.e. 'Hi Rakesh' is going from program to std::cout.
  ```cpp
  std::cout << "Hi Rakesh"; 
  ```
...
- While using 'cin' to input a string, C++ will not allow you to capture the string with spaces, it will assume the string with spaces as different variables. To avoid this use following -
  ```cpp
...
- From C++14 or newer compilers, apostrophes can be used to make very big numbers more readable.
...
- There are few things that you can do with floating point numbers that can not be done with integers such as
  ...
  2. A `0` floating point number can be divided by another `0` floating point number. This gives `NaN` - Not a number as a result but program will not crash.
...
- When initializing float variable, you need to append `f` suffix, for double append no prefix as it is default and for long double append `L` suffix.
...
- a single char occupies 1 byte in memory and hence can have '256' values in total. This is known as 'ASCII' encoding where each combination of 8 bits in a byte represents a separate character.
  ```cpp
  char a = 65;
...
- In C++, `std::string` and `std::string_view` are both used to represent and manipulate strings, but they serve different purposes and have different characteristics:
...
2. **std::string_view** 
    ...
    |----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
  	|	Ownership						 | 'std::string_view' does not own the string data. It simply provides a view over a part of an existing string, meaning it references memory owned by someone else.  |
...
- This library is used to get range of any given datatype.
  ```cpp
  // Returns minimum value that can be stored in datatype in argument. 
  // Returns the smallest positive value representable by the type if the type is a floating-point type. 
  // For integral types (like int or char), it returns the minimum representable value, which is the smallest (most negative) value if the type is signed.
  
  std::numeric_limits<int/char>::min()  
  
  // Returns maximum value that can be stored in datatype in argument.
  std::numeric_limits<int/char>::max()     
  
  // Returns maximum value that can be stored in datatype in argument.
  // Returns the lowest (most negative) finite value representable by the type. For floating-point types, lowest() is often equivalent to -max(). For integral types, lowest() is equivalent to min().

  std::numeric_limits<int/char>::lowest()  

  // Returns true for all other inbuilt data types except floating point data types.
...
- Switch statement
  1. The 'break' statement after each case is very important. It stops the processing after the a successful case has been found. If the 'break' statement is not there then program control will continue processing all the following case statements.

  2. Only Integral types: int, short int, long int etc. and enums are allowed as a condition in case statements. 'case' statements can also accept variables provided they are of 'const' type.
...
- 'size_t' is not a type in C++, It is just an alias for 'unsigned int'. It is generally used inside loop iterators, specifying sizes of the things etc. especially all the things that do not take negative values.
...
## 3. Arrays
- Arrays are collection of objects of same data type.
...
- char message [5] {'H', 'E', 'L', 'L', 'O'};
  ...
  ```
- You can not print character arrays directly in C++. Only strings can be printed directly in C++. 
- Strings are nothing but the character arrays that end with null characters i.e. '\0'
- Following will print garbage characters at the end.  
...
- compiler will automatically add trailing '\0' in the remaining part of the character array
  ```cpp
...
- Only character arrays can be printed directly, integer arrays will give compile time error.
...
- Bounds of an array
  ...
  2. If you go really crazy and use memory locations to far fetched than the array bounds, the C++ compiler will compile the program without any error but program will crash during runtime.
...
## 4. Pointers
- All the variables in C++ are stored in some memory locations. The address of these memory locations is stored in special type of variables which are known as 'pointer'.
...
- Declaring a pointer
  ...
  ```
- Initializing a pointer explicitly to nullptr, nullptr means that the pointer is not pointing to any memory location.
...
## 5. Virtual Memory
- A trick that fools your program to think that it is the only program that is running on your OS and all the memory resources belong to it.
...
- It is actually divided into sections such as -
...
- Stack
	1. Memory is finite.
	2. The developer is not in full control of memory lifetime.
	3. Lifetime is controlled by the scope mechanism.

- Heap
	1. Memory is finite.
	2. The developer is in full control of when memory is allocated and when it is released.
	3. Lifetime is controlled explicitly through new and delete operators.
...
- Following this also works, but no need to delete the "p_number" pointer twice.
  ```cpp
  #include <iostream>
...
- Dangling pointer  
  - A pointer that does not point to a valid memory address. Trying to dereference or use it will result in undefined behavior.
  - Following 3 types of behavior will lead to creation of dangling pointers 
  	1. Uninitialized pointer
		2. Deleted Pointer
		3. Multiple Pointers pointing to same memory location
...
- 'new' operator fail
  - The 'new' operator fails very rarely in practice. Many programs assume that 'new' operator always allocates the memory and do not check for its failure. 
  - Depending on the application failed memory allocations can be very bad and you should handle it.
...
- The new() operator failures are handled by two methods  
  ```cpp
  1. try-catch method     - new() fails are handled by catch block.
	2. std::nothrow method  - When new() fails, 'nullptr' is returned instead of raising exception.  
  ```
...
- This can happen in three ways
  ...
  2. Double allocation to a pointer
      ```cpp
			int *p_num1{new int{34}};   
	...
	3. Pointer in local scope i.e. Nested scope with dynamically allocated memory Whenever a pointer in an local scope is assigned address of memory then if the memory pointed by this pointer is not released before the pointer goes out of scope, a memory leak is created.
...
- If you modify the data through reference then the original variable will get modified and vice versa is also true i.e. when you modify the data through original variable the reference variable data will get modified as both are pointing to the same memory location.

### References Vs Pointers
- References 
	1. References do not need to be dereferenced for reading or writing the values stored in it.
	2. Must be initialized while declaring them and can not be changed to reference something else afterwards.
	3. Must be initialized at declaration time.

- Pointers
	1. Must undergo referencing or dereferencing to read or write the stored values.
	2. Can be changed to point somewhere else.
	3. Can be declared un-initialized (will contain garbage addresses.

- References behave like constant pointers but they have a much friendlier syntax as they don't require dereferencing to read and write through the referenced data.
...
- C-strings are not that easy or convenient to work with. You only need to keep track of how big the string is in memory, need to make sure that it ends correctly etc.
- C-strings do not provide many functions to manipulate strings. 
- C++ provides another type that builds on top of character arrays named - std::string but gives us easier interface to store strings and offer better string functionality.
- STL functions for manipulating characters -
  
  ```CPP
	std::isalnum()      // Check whether the argument character is alphanumeric or not i.e. 0-9 digits or upper/lower case alphabets.
	std::isalpha()      // Check whether the argument character is alphabet or not i.e. upper/lower case alphabet.
	std::isblank()      // Check whether the argument character is ' ' i.e. space character or not.
	std::isupper()      // returns True if argument character is in upper case.
	std::islower()      // returns True if argument character is in lower case.
	std::toupper()      // returns the upper case of the argument character.
	std::tolower()      // returns the lower case of the argument character.
  ```
...
- Strlen() Vs sizeof()
  ...
  3. strrchr() 
       - Return pointer to last occurrence of input character.
		- Returns the pointer to the last occurrence of the argument character in the string.

## 8.	C-string Concatination and Copying
- #include <cstring>
- std::strcat(dest, src) - char* strcat(char* dest, const char* src)
	1. join second string to the first string 
	2. You need to make sure that the first string is big enough to store both the strings.
	3. Because of these serious limitations, that you only need to keep track of the size of the first string, many compilers do not even allow to use this function.
...
### Declaring and using std::string
  ...
  - The pointers pointing to the string literal can be changed at runtime, However doing so will cause memory leak. 
  - As the memory used to store old string literal is still alloted to the program but not in use. 
  - This is not the case with std::strings. 
  - Dynamically reassigning strings will not cause memory leaks.
...
- Multiple definitions of a Class are allowed but in different translation units. 
- Following 'Point' class is also defined in file 'otherfile.cpp'. Defining 'Point' class in multiple files is allowed.
...
- Parameters passed without reference or pointer to the function are scoped locally in the function. 
- Changes to them are not visible outside the function. 
- That is what we have inside the function are actually copies of the arguments passed to the function.
...
### Pass By Value
1. Pass By Value  
  - When variables are passed by value to a function, local copy of variables is created in the function. 
  - And any changes to this local copy does not have any effect on the main variables.

2. Pass By Pointer  
	- The address of the variable is passed. The changes made in the function are actually done on the main variables.

3. Pass By Reference   
  - Reference (int&) are passed to the function. The changes to the function are actually done on the main variables. 
  - However, without the use of dereference operators. 
...
- They can be passed either by reference or by pointer. References are preferred in C++

- Input parameters should not be modifiable from inside the function. You enforce modification restrictions with const keyword. The input parameters can be passed either by const reference, passing by pointer to const or even passing by const pointer to const.
...
- No function will be called if there is ambiguity with type of parameters of the overloaded functions. 
...
- Function templates are just blueprints. They are not real C++ code consumed by the compiler. 
...
## 11. Template type deduction and explicit arguments
...
- While explicitly calling the template type function, if the parameter is of some different type then compiler will try to do type conversion implicitly. 
- However, if the parameter is not implicitly convertible then compiler error will be thrown.
...
- A static assert is a way to perform compile-time assertions. It ensures that certain conditions are met during compilation, preventing the program from compiling if the condition is false. 
...
- static asserts are used for following conditions - 
	1. Ensuring template parameters meet certain requirements.
	2. Checking for platform-specific conditions.
	3. Validating certain properties of types at compile time.
...
- 'Concepts' is a mechanism to place constraints on your template type parameters. 
- Using 'Concepts' we can specify that our function templateshould only be called with integer, double or string data types and if this is violated compiler error is generated.	
...
- Please not that, if we use 'require' keyword just once then the compiler will only check for the syntax but it will not enforce the semantic meaning of that statement
...
- Constructor 
	...
	3. has same name as class.
	4. can have parameters or can have empty parameter list.
	5. is usually used to initialize member variables of a class.

### Getters/Setters
- getter/setter are the class member functions defined by the user, which are public and are used to get/set the values of the private member variables of the class.
...
- when you split the code between .h file and a .cpp file make sure that all the definitions are moved to .cpp file and all the declarations are moved to .h file. 
...
- Destructors are called when 
	...
	2. When a heap object is release with delete keyword.
...
- Constant functions can only call other constant member functions and cannot modify the object's state.
  ...
  ```
	...
	1. The const keyword is placed after the parameter list but before the function body (or before the semicolon if it's a declaration).
	2. This signals that the function will not modify any member variables of the class.
...
- Key Points:
	1. The getData() function is declared as const, meaning it guarantees not to modify the internal state of the object.
	2. The constant member function getData() can be called on both constant and non-constant objects, whereas non-constant functions cannot be called on const objects.
...
- In C++, both friend functions and member functions are used to access and manipulate class data, but they have distinct differences in how they interact with class members, how they are called, and their purpose.
...
- Public Inheritance :-
  ...
  1. With public inheritance, derived class can have access of only public members of the base class and can not access private members of the base class.
	
  2. Same applied for the friends of the derived class. They can access all the members of the derived class but can access only public members of the base class and can not access private members of the base class.
	
  3. In Public inheritance, 'Protected' members of the base class are accessible to derived class member functions but are not accessible outside derived class.
	
  4. In public inheritance, the 'private', 'protected' and 'public' members of the base class remain same as 'private', 'protected' and 'public' in derived class. 
	    I think this statement is in the context where the derived class is used as an base class for next inheritance.
...
- Protected Inheritance :- 	
	1.	In protected inheritance, `public` and `protected` members of base class are inherited as `protected` members in derived class.
	2. `public` members of base class are accessible both inside and outside the member functions of the derived class.
	3. `protected` members of base class are accessible only inside the member functions of the derived class.
	4.  However, `private` members of base class are not accessible even inside the member functions of the derived class, in any type of inheritance.
...
- Private Inheritance :- 	
	1. In private inheritance, `public` and `protected` members of base class are inherited as `private` members in derived class.
	2. This means that `public` and `protected` members of base class can be accessed inside the member functions of the derived class but not outside.
	3. However, `private` members of base class are not accessible even inside the member functions of the derived class, in any type of inheritance.
	4. The class that does `private` inheritance is really selfish as it does not forward anything that is coming from the base class to the class which derives from it.
...
- In C++, you might encounter situations where a member variable or function of a base class is hidden or overshadowed in a derived class due to inheritance, particularly when member functions in the derived class have the same name as those in the base class. This is known as name hiding or member overshadowing.

- To resurrect or bring back members from the base class into the scope of the derived class, you can explicitly use the scope resolution operator (::) or the using declaration.
...
- The constructors are called for the base class first and then for the derived classes whereas destructors are called in the reverse order to that of constructors. 
- Destructor for derived class is called first and then for the base classes.
		
### Reused Symbols in Inheritance
- If you define a member in a derived class with same name/signature as that in the base class, C++ will give preference to the member defined in the derived class i.e. members in child class are going to hide the members in the parent class.
...
## 19. Polymorphism
- Polymorphism is all about accessing the different derived class objects using the pointer or reference of the base class from which they are derived and still calling the respective methods of these derived classes.
...
- If we prefix the functions, which have same signature and are member of derived classes, with 'virtual' keyword, then this is called as 'Dynamic' binding.

- In dynamic binding, the base class pointer can be used to store the objects of the derived classes and when virtual function is invoked using this pointer, 
the function related to the object class will be called instead of calling the function depending on the type of the pointer.
...
- Object Slicing   
  - When you assign a derived-class-object directly to the base-class type variable, the derived object is sliced off, the compiler copies only base-class part of that derived-class object into that variable. 
...
- In C++, creating an array of references is not allowed. 
- This restriction arises from the nature of references themselves, which are not objects and do not occupy storage space like regular variables do. 
...
- In C++, smart pointers are used to automatically manage the lifetime of dynamically allocated objects. 
- The two most commonly used types are 'std::unique_ptr' and 'std::shared_ptr', both part of the C++11 standard and beyond. 
- These help avoid memory leaks and ensure that resources are properly released when they are no longer needed.
...
- Once the 'unique_ptr' is destroyed, the memory allocated for the object is automatically released (freed).
- Key Properties:
	1. Exclusive ownership: Only one unique_ptr can own an object.
	2. Cannot be copied, but can be moved to transfer ownership to another unique_ptr.
	3. Automatically deletes the object when the unique_ptr goes out of scope.
    ...
    ```
- Explanation:
	- The unique_ptr ensures that only one smart pointer owns the object at any time. When ptr1 is moved to ptr2, ptr1 becomes a nullptr, and ptr2 now owns the object. 
	- The destructor is automatically called when the unique_ptr goes out of scope.

### 2. std::shared_ptr
- A shared_ptr is a smart pointer that manages a dynamically allocated object through reference counting. 
- Multiple shared_ptr instances can point to the same object, and the object is only destroyed when the last shared_ptr pointing to it goes out of scope (or is reset).
- Key Properties:
	1. Shared ownership: Multiple shared_ptr can own the same object.
	2. Uses reference counting to keep track of how many shared_ptr instances are pointing to the object.
	3. When the reference count drops to zero (i.e., all shared_ptr have been destroyed or reset), the object is automatically deleted.
...
- C++ References are not left assignable, i.e. once declared they can not be modified to point to the another object. 
- Doing so, will not modify the reference but it will modify the object that the reference is pointing to.
...
### "Override" keyword  
- In C++, the 'override' keyword is used in inheritance to indicate that a member function in a derived class is meant to override a virtual function in a base class. 
...
- Benefits of override:
	...
	1. Compile-time safety: 	The compiler checks if a function is correctly overriding a virtual function. If the base class function is not virtual, 
														or if the signatures don’t match, the compiler will throw an error.
...
- Shape -> Oval -> Circle
- If we have a virtual function that is overloaded, then defining only one of them with keyword 'override' and not others will result in getting other member functions that are overloads of this function get masked and become unavailable for the downstream classes.
			e.g. - 
...
- 'virtual' and 'final' are mutually contradicting ideas. 'Virtual' means you want to specialize that method in derived classes. 
- Whereas 'final' means you do not want to specialize it in downstream classes. 
- In both of the specializers 'final' will be given more preference.
...
- Dynamic casts are going to work only with polymorphic inheritance hierarchies i.e. transformations are allowed between the pointers/references of the base and derived class. 
...
- If you call a virtual function from constructor/destructor the call will never go to a more derived class than the currently executing class.
- Calling a virtual function from a constructor or destructor won't give you polymorphic results.
- If you call virtual functions from the constructor or destructor you are going to get static binding results, this is bcz if you call a virtual function in Base constructor, till that time derived object would have not been created yet. 
- And similarly, if you call virtual function in base destructor, derived object's destructor would have been called and derived object would have been destroyed already.
...
- Derived classes from an abstract class must explicitly override all the pure virtual functions from the abstract parent class, if they don't they themselves will become an abstract class.
...
## 21. typeid() in C++
- In C++, the typeid operator is used to obtain the type information of an expression or an object at runtime. It is part of the Run-Time Type Information (RTTI) system in C++. 
...
- Key Features of typeid:
	1. It is used to query the type of an object or a type.
	2. It can be used with both basic types and user-defined types.
...
- Requirements:
	...
	2. If the expression is polymorphic (involving virtual functions), typeid returns the dynamic type (the type determined at runtime).
...
- Example 3: Comparing Types
  ...
  ```
    ...
    4. **Limitations**  
        ...
        1. Compiler-Dependent Output   
            - The name() function of std::type_info returns a string that is not standardized and is implementation-specific.   
            - For instance, different compilers may produce different strings for the same type.
        
        2. Only Works with Polymorphic Types at Runtime  
